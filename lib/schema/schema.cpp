...
static std::error_code check_number (peejay::element const &el,
                                     Predicate pred) {
...
std::error_code number_constraints (peejay::object const &schema,
  ...
  if (auto const minimum_pos = schema->find (u8"minimum"); minimum_pos != end) {
    if (auto const erc = check_number (minimum_pos->second, [num] (
                                                                auto const &v) {
  ...
  if (auto const exmin_pos = schema->find (u8"exclusiveMinimum");
      exmin_pos != end) {
    if (auto const erc = check_number (exmin_pos->second, [num] (
                                                              auto const &v) {
          using target_type = math_type<NumberType, std::decay_t<decltype (v)>>;
          return static_cast<target_type> (num) > static_cast<target_type> (v);
        })) {
...
std::error_code schema::checker::check_type (peejay::u8string const &type_name,
                                             peejay::element const &instance) {
  static std::unordered_map<peejay::u8string,
                            std::function<bool (peejay::element const &)>> const
      map{
...
std::error_code schema::checker::check_type (element const &type_name,
...
std::error_code schema::checker::string_constraints (object const &schema,
                                                     u8string const &s) {
  auto const end = schema->end ();
...
std::error_code schema::checker::object_constraints (object const &schema,
                                                     object const &obj) {
  auto const end = schema->end ();
...
std::error_code schema::checker::check (element const &schema,
                                        element const &instance) {
...
std::error_code schema::checker::root (element const &schema,
                                       element const &instance) {
  root_ = &schema;
