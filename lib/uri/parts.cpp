...
std::size_t pct_encoded_size(std::string_view const str, pctencode_set const encodeset) {
  if (!uri::needs_pctencode(str, encodeset)) {
    return std::size_t{0};
  }
  ro_sink_container<char> sink;
  uri::pctencode(std::begin(str), std::end(str), std::back_inserter(sink), encodeset);
  return sink.size();
}
