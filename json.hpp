...
#define JSON_FALLTHROUGH //__attribute__ ((fallthrough)) GCC 7 required?
...
namespace json {
    ...
    public:
        ...
        result_type parse (std::string const & src);
    ...
    // ~~~~~
    template <typename Callbacks>
    auto parser<Callbacks>::parse (std::string const & str) -> result_type {
    ...
    auto parser<Callbacks>::eof () -> result_type {
        ...
        return has_error () ? default_return<result_type>::get () : this->callbacks ().result ();
    }
    ...
    public:
        ...
        /// \returns True if this matcher has completed (and reached it's "done" state). The parser
        /// should pop this instance from the parse stack before continuing.
    ...
    public:
        null_token_matcher (parser<Callbacks> & parser)
                : token_matcher<Callbacks> (
                      parser, "null", std::bind (&Callbacks::null_value, &parser.callbacks ())) {}
    };
    ...
    void number_matcher<Callbacks>::do_frac_state (char c, source & s) {
        if (c == '.') {
            ...
            // digits are definitely not part of the next token so we can issue an error right here.
            this->set_error (error_code::number_out_of_range);
        } else {
    ...
    std::unique_ptr<matcher<Callbacks>> string_matcher<Callbacks>::consume (maybe<char> ch,
        ...
        if (complete) {
            ...
            case hex2_state: JSON_FALLTHROUGH;
            case hex3_state:
    ...
    std::unique_ptr<matcher<Callbacks>> object_matcher<Callbacks>::consume (maybe<char> ch,
        ...
        case key_state:
            ...
            return matcher_ptr (
                new root_matcher<Callbacks> (this->get_parser (), true /*only string allowed*/));
        case colon_state:
            if (source::isspace (c)) {
    ...
    std::unique_ptr<matcher<Callbacks>> root_matcher<Callbacks>::consume (maybe<char> ch,
        ...
        case new_token_state: {
            ...
            case 't': return matcher_ptr (new true_token_matcher<Callbacks> (this->get_parser ()));
            case 'f': return matcher_ptr (new false_token_matcher<Callbacks> (this->get_parser ()));
            case 'n': return matcher_ptr (new null_token_matcher<Callbacks> (this->get_parser ()));
