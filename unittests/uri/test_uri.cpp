...
// NOLINTNEXTLINE
TEST(UriSplit, 0001) {
  auto const x = uri::split_reference("C://[::A:eE5c]:2194/&///@//:_/%aB//.////#");
  ASSERT_TRUE(x);
  ...
  EXPECT_EQ(static_cast<std::filesystem::path>(x->path).generic_string(), "/&/@/:_/%aB/./");
}
// NOLINTNEXTLINE
TEST(UriSplit, 0002) {
...
TEST(UriSplit, 0004) {
  ...
  EXPECT_EQ(static_cast<std::filesystem::path>(x->path).generic_string(), "%Cf");
}
// NOLINTNEXTLINE
TEST(UriSplit, 0005) {
  auto const x = uri::split_reference("E07:/8=-~%bF//%36////'/%16N%78//)/%53/;?*!");
  ASSERT_TRUE(x);
  EXPECT_EQ(x->scheme, "E07");
  EXPECT_FALSE(x->authority);
  EXPECT_TRUE(x->path.absolute);
  EXPECT_THAT(x->path.segments, ElementsAre("8=-~%bF", "", "%36", "", "", "", "'", "%16N%78", "", ")", "%53", ";"));
  EXPECT_EQ(x->query, "*!");
  EXPECT_FALSE(x->fragment);
}
// NOLINTNEXTLINE
...
TEST(UriSplit, 0010) {
  ...
  EXPECT_FALSE(x->authority);
  EXPECT_FALSE(x->path.absolute);
  EXPECT_THAT(x->path.segments, ElementsAre("d", "M", "kx", "s", "GTl", "", "", "SgA", ""));
  EXPECT_EQ(x->query, "");
  EXPECT_EQ(x->fragment, "");
}
// NOLINTNEXTLINE
...
// NOLINTNEXTLINE
TEST(UriSplit, 0032) {
  auto const x = uri::split_reference(R"(y://%DD@253.216.255.251//aa/??/://;)");
  ASSERT_TRUE(x);
  EXPECT_EQ(x->scheme, "y");
  ASSERT_TRUE(x->authority.has_value());
...
// NOLINTNEXTLINE
TEST(UriSplit, 0033) {
  auto const x = uri::split_reference("B://.@[AC::1:6DEb:14.97.229.249]:?/#??~(");
  ASSERT_TRUE(x);
...
// NOLINTNEXTLINE
TEST(UriSplit, 0047) {
  auto const x = uri::split_reference("C-://[::1E:BB:a:5c1:Dd:40.44.228.108]/;?#");
  ASSERT_TRUE(x);
  EXPECT_EQ(x->scheme, "C-");
  ASSERT_TRUE(x->authority.has_value());
  EXPECT_FALSE(x->authority->userinfo);
...
// NOLINTNEXTLINE
TEST(UriSplit, 0048) {
  auto const x = uri::split_reference("z://[c:BC:b:A:Bd:D:dC1f:cedB]?/#/:/%FA");
  ASSERT_TRUE(x);
...
// NOLINTNEXTLINE
TEST(UriSplit, 0053) {
  auto const x = uri::split_reference("A://[vA5.+:=.p~=)=&_;-=7)(.;]:768295/+");
  ASSERT_TRUE(x);
  EXPECT_EQ(x->scheme, "A");
  ASSERT_TRUE(x->authority.has_value());
  EXPECT_FALSE(x->authority->userinfo);
...
// NOLINTNEXTLINE
TEST(UriSplit, 0062) {
  auto const x = uri::split_reference("f.://[d1b:CF:AbBa::F:d:11.246.155.253]?");
  ASSERT_TRUE(x);
...
// NOLINTNEXTLINE
TEST(UriSplit, 0069) {
  auto const x = uri::split_reference(R"(v+://@[::C:dEd:4:218.255.251.5]:8/@.;J??Q??%48/#)");
  ASSERT_TRUE(x);
  EXPECT_EQ(x->scheme, "v+");
  ASSERT_TRUE(x->authority.has_value());
  EXPECT_EQ(x->authority->userinfo, "");
...
// NOLINTNEXTLINE
TEST(UriSplit, 0077) {
  auto const x = uri::split_reference(R"(Y://[4Bbc:bb::cDcd:5:c4:e:B1]:/%CA@/./??)");
  ASSERT_TRUE(x);
  EXPECT_EQ(x->scheme, "Y");
  ASSERT_TRUE(x->authority.has_value());
  EXPECT_FALSE(x->authority->userinfo);
...
TEST(UriFileSystemPath, Root) {
  ...
  p.absolute = true;
  EXPECT_EQ(static_cast<std::filesystem::path>(p), std::filesystem::path("/"));
}
// NOLINTNEXTLINE
TEST(UriFileSystemPath, RootFoo) {
  ...
  EXPECT_EQ(static_cast<std::filesystem::path>(p), std::filesystem::path("/foo"));
}
// NOLINTNEXTLINE
TEST(UriFileSystemPath, AbsoluteTwoSegments) {
  ...
  EXPECT_EQ(static_cast<std::filesystem::path>(p), std::filesystem::path("/foo/bar"));
}
// NOLINTNEXTLINE
...
TEST_F(Join, Abnormal) {
  ...
  EXPECT_EQ(uri::split("http://a/b/c/g;x=1/y"), uri::join(base_, "g;x=1/./y"));
...
TEST(PartsValid, SchemeMixedCharacters) {
  uri::parts p;
  p.scheme = "a+123";
  EXPECT_TRUE(p.valid()) << R"!(scheme must be ALPHA *( ALPHA / DIGIT / "+" / "-" / "." ))!";
}
