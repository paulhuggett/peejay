...
TEST (PctEncode, Hello) {
  ...
  uri::pctencode (std::begin (input), std::end (input),
                  std::back_inserter (out), uri::pctencode_set::component);
  EXPECT_EQ (out, "Hello");
}
...
TEST (PctEncode, CodePointNeedsEncodeExhaustive) {
  ...
  auto const is_query = [=] (uint_least8_t c) {
    return is_c0 (c) || c == 0x20 || c == 0x22 || c == 0x23 || c == 0x3C ||
           c == 0x3E;
  };
  // The special-query percent-encode set is the query percent-encode set and
  // U+0027 (').
  auto const is_special_query = [=] (uint_least8_t c) {
    return is_query (c) || c == 0x27;
  };
  // The path percent-encode set is the query percent-encode set and U+003F (?),
  // U+0060 (`), U+007B ({), and U+007D (}).
  auto const is_path = [=] (uint_least8_t c) {
    return is_query (c) || c == 0x3F || c == 0x60 || c == 0x7B || c == 0x7D;
  };
  ...
  auto const is_userinfo = [=] (uint_least8_t c) {
    return is_path (c) || c == 0x2F || c == 0x3A || c == 0x3B || c == 0x3D ||
           c == 0x40 || c == 0x5B || c == 0x5C || c == 0x5D || c == 0x5E ||
           c == 0x7C;
  };
  ...
  auto const is_form_urlencoded = [=] (uint_least8_t c) noexcept {
    return is_component (c) || c == 0x21 || c == 0x27 || c == 0x28 ||
           c == 0x29 || c == 0x7E;
  };
  ...
  chars.resize (0xFFU);
  std::iota (std::begin (chars), std::end (chars),
             std::numeric_limits<uint_least8_t>::min ());
  for (auto const c : chars) {
    EXPECT_EQ (uri::needs_pctencode (c, uri::pctencode_set::fragment),
               is_fragment (c));
    EXPECT_EQ (uri::needs_pctencode (c, uri::pctencode_set::query),
               is_query (c));
    EXPECT_EQ (uri::needs_pctencode (c, uri::pctencode_set::special_query),
               is_special_query (c));
    EXPECT_EQ (uri::needs_pctencode (c, uri::pctencode_set::path), is_path (c));
    EXPECT_EQ (uri::needs_pctencode (c, uri::pctencode_set::userinfo),
               is_userinfo (c));
    EXPECT_EQ (uri::needs_pctencode (c, uri::pctencode_set::component),
               is_component (c));
    EXPECT_EQ (uri::needs_pctencode (c, uri::pctencode_set::form_urlencoded),
               is_form_urlencoded (c));
  }
}
