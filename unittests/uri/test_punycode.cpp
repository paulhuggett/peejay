...
TEST(Punycode, ArabicEgyptian) {
  // Arabic (Egyptian)
  auto const arabic = std::u32string{0x0644, 0x064A, 0x0647, 0x0645, 0x0627, 0x0628, 0x062A, 0x0643, 0x0644,
  ...
  EXPECT_EQ(std::get<decoded_success_result_index>(decoded).in, std::end(encoded_arabic));
}

// NOLINTNEXTLINE
TEST(Punycode, ChineseSimplified) {
  // Chinese (simplified)
  auto const chinese_simplified =
      std::u32string{0x4ED6, 0x4EEC, 0x4E3A, 0x4EC0, 0x4E48, 0x4E0D, 0x8BF4, 0x4E2D, 0x6587};
  auto const encoded_chinese_simplified = "ihqwcrb4cv8a8dqg056pqjye"sv;
  std::string actual;
  uri::punycode::encode(chinese_simplified, false, std::back_inserter(actual));
  EXPECT_EQ(actual, encoded_chinese_simplified);

  auto const decoded = uri::punycode::decode(encoded_chinese_simplified);
  ASSERT_EQ(decoded.index(), decoded_success_result_index);
  EXPECT_EQ(std::get<decoded_success_result_index>(decoded).str, chinese_simplified);
  EXPECT_EQ(std::get<decoded_success_result_index>(decoded).in, std::end(encoded_chinese_simplified));
}

// NOLINTNEXTLINE
TEST(Punycode, ChineseTraditional) {
  // Chinese (traditional)
  auto const orig = std::u32string{0x4ED6, 0x5011, 0x7232, 0x4EC0, 0x9EBD, 0x4E0D, 0x8AAA, 0x4E2D, 0x6587};
...
TEST(Punycode, Czech) {
  ...
  auto const orig =
      std::u32string{0x0050, 0x0072, 0x006F, 0x010D, 0x0070, 0x0072, 0x006F, 0x0073, 0x0074, 0x011B, 0x006E,
...
TEST(Punycode, Hebrew) {
  auto const orig =
      std::u32string{0x05DC, 0x05DE, 0x05D4, 0x05D4, 0x05DD, 0x05E4, 0x05E9, 0x05D5, 0x05D8, 0x05DC, 0x05D0,
...
TEST(Punycode, HindiDevanagari) {
  auto const orig = std::u32string{0x092F, 0x0939, 0x0932, 0x094B, 0x0917, 0x0939, 0x093F, 0x0928, 0x094D, 0x0926,
...
TEST(Punycode, JapaneseKanjiAndHiragana) {
  auto const orig = std::u32string{0x306A, 0x305C, 0x307F, 0x3093, 0x306A, 0x65E5, 0x672C, 0x8A9E, 0x3092,
...
TEST(Punycode, KoreanHangulSyllables) {
  auto const orig =
      std::u32string{0xC138, 0xACC4, 0xC758, 0xBAA8, 0xB4E0, 0xC0AC, 0xB78C, 0xB4E4, 0xC774, 0xD55C, 0xAD6D, 0xC5B4,
                     0xB97C, 0xC774, 0xD574, 0xD55C, 0xB2E4, 0xBA74, 0xC5BC, 0xB9C8, 0xB098, 0xC88B, 0xC744, 0xAE4C};
  auto const encoded = "989aomsvi5e83db1d2a355cv1e0vak1dwrv93d5xbh15a0dt30a5jpsd879ccm6fea98c"sv;
  std::string actual;
  uri::punycode::encode(orig, false, std::back_inserter(actual));
  EXPECT_EQ(actual, encoded);
...
TEST(Punycode, RussianCyrillic) {
  auto const orig = std::u32string{0x043F, 0x043E, 0x0447, 0x0435, 0x043C, 0x0443, 0x0436, 0x0435, 0x043E, 0x043D,
                                   0x0438, 0x043D, 0x0435, 0x0433, 0x043E, 0x0432, 0x043E, 0x0440, 0x044F, 0x0442,
                                   0x043F, 0x043E, 0x0440, 0x0443, 0x0441, 0x0441, 0x043A, 0x0438};
...
TEST(Punycode, Spanish) {
  ...
  auto const orig = std::u32string{0x0050, 0x006F, 0x0072, 0x0071, 0x0075, 0x00E9, 0x006E, 0x006F, 0x0070, 0x0075,
                                   0x0065, 0x0064, 0x0065, 0x006E, 0x0073, 0x0069, 0x006D, 0x0070, 0x006C, 0x0065,
                                   0x006D, 0x0065, 0x006E, 0x0074, 0x0065, 0x0068, 0x0061, 0x0062, 0x006C, 0x0061,
                                   0x0072, 0x0065, 0x006E, 0x0045, 0x0073, 0x0070, 0x0061, 0x00F1, 0x006F, 0x006C};
...
TEST(Punycode, Vietnamese) {
  ...
  auto const orig =
      std::u32string{0x0054, 0x1EA1, 0x0069, 0x0073, 0x0061, 0x006F, 0x0068, 0x1ECD, 0x006B, 0x0068, 0x00F4,
...
TEST(Punycode, ExampleL) {
  ...
  auto const orig = std::u32string{0x0033, 0x5E74, 0x0042, 0x7D44, 0x91D1, 0x516B, 0x5148, 0x751F};
...
TEST(Punycode, ExampleM) {
  ...
  auto const orig =
      std::u32string{0x5B89, 0x5BA4, 0x5948, 0x7F8E, 0x6075, 0x002D, 0x0077, 0x0069, 0x0074, 0x0068, 0x002D, 0x0053,
...
TEST(Punycode, ExampleN) {
  ...
  auto const orig = std::u32string{0x0048, 0x0065, 0x006C, 0x006C, 0x006F, 0x002D, 0x0041, 0x006E, 0x006F,
...
TEST(Punycode, ExampleO) {
  ...
  auto const orig = std::u32string{0x3072, 0x3068, 0x3064, 0x5C4B, 0x6839, 0x306E, 0x4E0B, 0x0032};
...
TEST(Punycode, ExampleP) {
  ...
  auto const orig = std::u32string{0x004D, 0x0061, 0x006A, 0x0069, 0x3067, 0x004B, 0x006F,
...
TEST(Punycode, ExampleQ) {
  //  <pafii>de<runba>
  auto const orig = std::u32string{0x30D1, 0x30D5, 0x30A3, 0x30FC, 0x0064, 0x0065, 0x30EB, 0x30F3, 0x30D0};
...
TEST(Punycode, ExampleR) {
  // <sono><supiido><de>
  auto const orig = std::u32string{0x305D, 0x306E, 0x30B9, 0x30D4, 0x30FC, 0x30C9, 0x3067};
  auto const encoded = "d9juau41awczczp"sv;
  std::string actual;
  auto const [_1, _2, has_non_basic] = uri::punycode::encode(orig, false, std::back_inserter(actual));
  EXPECT_EQ(actual, encoded);
...
TEST(Punycode, ExampleS) {
  // (S) -> $1.00 <-
  auto const orig = std::u32string{'-', '>', ' ', '$', '1', '.', '0', '0', ' ', '<', '-'};
...
void DecodeEncodeRoundTrip(std::string_view original) {
  ...
  if (res.index() == decoded_success_result_index) {
    std::string encoded;
    uri::punycode::encode(std::get<decoded_success_result_index>(res).str, false, std::back_inserter(encoded));
    ...
    EXPECT_TRUE(std::equal(std::begin(original), ascii_end, std::begin(encoded)));
    EXPECT_TRUE(
        (std::equal(ascii_end, std::end(original), std::begin(encoded) + std::distance(std::begin(original), ascii_end),
