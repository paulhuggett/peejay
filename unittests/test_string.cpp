...
TEST_F (JsonString, Empty) {
  ...
  EXPECT_EQ (p.coordinate (), (peejay::coord{3U, 1U}));
}
...
TEST_F (JsonString, Simple) {
  ...
  EXPECT_FALSE (p.has_error ()) << "Expected the parse to succeed";
  EXPECT_FALSE (p.last_error ()) << "Expected the parse error to be zero";
  EXPECT_EQ (p.coordinate (), (peejay::coord{8U, 1U}));
}
...
TEST_F (JsonString, Unterminated) {
  ...
  EXPECT_EQ (p.last_error (),
             make_error_code (peejay::error_code::expected_close_quote));
...
TEST_F (JsonString, EscapeN) {
  ...
  EXPECT_FALSE (p.has_error ()) << "Expected the parse to succeed";
  EXPECT_FALSE (p.last_error ()) << "Expected the parse error to be zero";
  EXPECT_EQ (p.coordinate (), (peejay::coord{6U, 1U}));
}
...
TEST_F (JsonString, BadEscape1) {
  ...
  EXPECT_EQ (p.last_error (),
             make_error_code (peejay::error_code::invalid_escape_char));
  EXPECT_EQ (p.coordinate (), (peejay::coord{4U, 1U}));
}
...
TEST_F (JsonString, BackslashQuoteUnterminated) {
  ...
  EXPECT_EQ (p.last_error (),
             make_error_code (peejay::error_code::expected_close_quote));
  EXPECT_EQ (p.coordinate (), (peejay::coord{5U, 1U}));
}
...
TEST_F (JsonString, SlashUnicodeUpper) {
  ...
  EXPECT_FALSE (p.has_error ()) << "Expected the parse to succeed";
  EXPECT_FALSE (p.last_error ()) << "Expected the parse error to be zero";
  EXPECT_EQ (p.coordinate (), (peejay::coord{9U, 1U}));
}
...
TEST_F (JsonString, TwoUtf16Chars) {
  ...
  EXPECT_FALSE (p.has_error ()) << "Expected the parse to succeed";
  EXPECT_FALSE (p.last_error ()) << "Expected the parse error to be zero";
  EXPECT_EQ (p.coordinate (), (peejay::coord{15U, 1U}));
}
...
TEST_F (JsonString, Utf16HighWithNoLowSurrogate) {
  ...
  p.input (R"("\uD834\u30A1")"s).eof ();
  EXPECT_EQ (p.last_error (),
             make_error_code (peejay::error_code::bad_unicode_code_point));
  EXPECT_EQ (p.coordinate (), (peejay::coord{13U, 1U}));
}
...
TEST_F (JsonString, SlashBadHexChar) {
  ...
  p.input ("\"\\u00xF\""s).eof ();
  EXPECT_EQ (p.last_error (),
             make_error_code (peejay::error_code::invalid_hex_char));
  EXPECT_EQ (p.coordinate (), (peejay::coord{6U, 1U}));
}
