//===- unittests/peejay/test_number.cpp -----------------------------------===//
//*                        _                *
...
TEST_F(Number, HexExtensionDisabled) {
  ...
  input(p, u8"0x10"sv).eof();
  EXPECT_TRUE(p.has_error());
  EXPECT_EQ(p.last_error(), make_error_code(error::number_out_of_range)) << "Error was: " << p.last_error().message();
}
// NOLINTNEXTLINE
TEST_F(Number, BadLetterAfterX) {
  ...
  EXPECT_TRUE(p.has_error());
  EXPECT_EQ(p.last_error(), make_error_code(error::expected_digits)) << "Error was: " << p.last_error().message();
}
// NOLINTNEXTLINE
...
TEST_F(Number, Infinity) {
  EXPECT_CALL(callbacks_, double_value(std::numeric_limits<double>::infinity())).Times(1);
  auto p = make_parser(proxy_, extensions::numbers);
...
TEST_F(Number, InfinityExtensionDisabled) {
  ...
  input(p, u8"Infinity"sv).eof();
  EXPECT_TRUE(p.has_error());
  EXPECT_EQ(p.last_error(), make_error_code(error::expected_token)) << "Error was: " << p.last_error().message();
}
// NOLINTNEXTLINE
TEST_F(Number, NaN) {
...
TEST_F(Number, PlusInfinity) {
  EXPECT_CALL(callbacks_, double_value(DoubleEq(std::numeric_limits<double>::infinity()))).Times(1);
...
TEST_F(Number, MinusInfinity) {
  EXPECT_CALL(callbacks_, double_value(DoubleEq(-1.0 * std::numeric_limits<double>::infinity()))).Times(1);
  auto p = make_parser(proxy_, extensions::all);
  input(p, u8"-Infinity"sv).eof();
  EXPECT_FALSE(p.last_error()) << "Expected the parse error to be zero. Was: " << p.last_error().message();
}
