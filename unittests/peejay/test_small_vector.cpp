//===- unittests/peejay/test_small_vector.cpp -----------------------------===//
//*                      _ _                  _              *
...
// 2/3: copy_thorws
static_assert(std::is_nothrow_constructible_v<peejay::small_vector<copy_throws>>);
static_assert(std::is_nothrow_move_constructible_v<peejay::small_vector<copy_throws>>);
static_assert(std::is_nothrow_move_assignable_v<peejay::small_vector<copy_throws>>);
// 3/3: move_throws
static_assert(std::is_nothrow_constructible_v<peejay::small_vector<move_throws>>);
static_assert(!std::is_nothrow_move_constructible_v<peejay::small_vector<move_throws>>);
static_assert(!std::is_nothrow_move_assignable_v<peejay::small_vector<move_throws>>);
...
TEST(SmallVector, CtorCountFirstLastInputIteratorInBody) {
  ...
  peejay::small_vector<int, 4> const v{std::istream_iterator<int>{src}, std::istream_iterator<int>{}};
  EXPECT_THAT(v, ElementsAre(2, 3, 5, 7));
}
// NOLINTNEXTLINE
...
TEST(SmallVector, CtorCountFirstLastInputIteratorLarge) {
  std::istringstream src("2 3 5 7");
  peejay::small_vector<int, 2> const v{std::istream_iterator<int>{src}, std::istream_iterator<int>{}};
  EXPECT_THAT(v, ElementsAre(2, 3, 5, 7));
}
...
TEST(SmallVector, SizeAfterResizeLarger) {
  ...
  EXPECT_GE(size, b.capacity()) << "expected capacity to be at least " << size << " (the container size)";
}

// NOLINTNEXTLINE
TEST(SmallVector, ResizeLargeToLarge) {
  peejay::small_vector<int, 2> b(std::size_t{4});
  EXPECT_GE(b.capacity(), b.size()) << "expected capacity to be at least 4 (the container size)";
  constexpr auto size = std::size_t{5};
  b.resize(size);
  EXPECT_EQ(b.size(), size);
  EXPECT_GE(b.capacity(), size) << "expected capacity to be at least " << size << " (the container size)";
}
...
TEST(SmallVector, IteratorConstFromNonConstContainer) {
...

  {
    ...
    std::vector<int> actual;
    // NOLINTNEXTLINE(modernize-loop-convert)
    for (decltype(buffer)::const_iterator it = buffer.cbegin(), end = buffer.cend(); it != end; ++it) {
      actual.push_back(*it);
    }
    EXPECT_THAT(actual, ::testing::ElementsAre(42, 43, 44, 45));
  }
...
template <typename TypeParam> class SmallVectorErase : public testing::Test {};

using Sizes = testing::Types<std::integral_constant<std::size_t, 2>, std::integral_constant<std::size_t, 3>,
...
TEST(SmallVector, VariantIsValuelessByException) {
  ...
  EXPECT_EQ(sv.size(), sv.body_elements()) << "The 'small' container is full";
  ...
  // valid in this case.
  // NOLINTNEXTLINE
  EXPECT_THROW(sv.emplace_back(throws_on_cast_to_int{}), throws_on_cast_to_int::ex);
