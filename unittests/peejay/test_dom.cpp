...
protected:
  element *element::*object_parent_field = &object::element_type::value_type::second_type::parent;
...
TEST_F(Dom, Array) {
  ...
  ASSERT_THAT(arr, ElementsAre(VariantWith<std::int64_t>(1), VariantWith<std::int64_t>(2)));
...
TEST_F(Dom, Array2) {
  ...
  ASSERT_THAT(arr,
              ElementsAre(VariantWith<u8string>(u8string{reinterpret_cast<char8 const *>(expected_bytes.data())})));
  // Check the parent pointers.
  EXPECT_EQ(root->parent, nullptr);
  EXPECT_THAT(arr, Each(Field("parent", parent_field, Eq(&root.value()))));
}
...
TEST_F(Dom, Object) {
  ...
  EXPECT_THAT(root_element, UnorderedElementsAre(Pair(u8"a"s, VariantWith<std::int64_t>(1)),
                                                 Pair(u8"b"s, VariantWith<std::int64_t>(2))));
  // Check the parent pointers.
  EXPECT_EQ(root->parent, nullptr);
  // Object members have the root object as parent.
  EXPECT_THAT(root_element, Each(Pair(_, Field("parent", object_parent_field, Eq(&root.value())))));
}
// NOLINTNEXTLINE
TEST_F(Dom, ObjectInsideArray1) {
  ...
  ASSERT_THAT(root, Optional(VariantWith<array>(_)));
  auto const &arr = *std::get<array>(*root);
  ASSERT_THAT(arr, ElementsAre(VariantWith<object>(_), VariantWith<std::int64_t>(3)));
  EXPECT_THAT(*std::get<object>(arr.at(0)), UnorderedElementsAre(Pair(u8"a"s, VariantWith<std::int64_t>(1)),
                                                                 Pair(u8"b"s, VariantWith<std::int64_t>(2))));

  // The array elements have the root object as parent.
  EXPECT_THAT(arr, Each(Field("parent", parent_field, Eq(&root.value()))));
  // arr[0] members have the arr[0] object as parent.
  EXPECT_THAT(*std::get<object>(arr.at(0)), Each(Pair(_, Field("parent", object_parent_field, Eq(&arr.at(0))))));
}
// NOLINTNEXTLINE
TEST_F(Dom, ObjectInsideArray2) {
  ...
  ASSERT_THAT(root, Optional(VariantWith<array>(_)));
  auto const &arr = *std::get<array>(*root);
  ASSERT_THAT(arr, ElementsAre(VariantWith<std::int64_t>(1), VariantWith<object>(_)));
  auto const &ind1 = *std::get<object>(arr.at(1));
  EXPECT_THAT(ind1, UnorderedElementsAre(Pair(u8"a"s, VariantWith<std::int64_t>(2)),
                                         Pair(u8"b"s, VariantWith<std::int64_t>(3))));

  // The array elements have the root object as parent.
  EXPECT_THAT(arr, Each(Field("parent", parent_field, Eq(&root.value()))));
  // arr[1] members have the arr[0] object as parent.
  EXPECT_THAT(ind1, Each(Pair(_, Field("parent", object_parent_field, Eq(&arr.at(1))))));
}
...
TEST_F(Dom, ArrayInsideObject) {
  ...
  ASSERT_THAT(obj,
              UnorderedElementsAre(Pair(u8"a"s, VariantWith<array>(_)), Pair(u8"b"s, VariantWith<std::int64_t>(3))));
...
TEST_F(Dom, DuplicateKeys) {
  ...
  ASSERT_THAT(root, Optional(VariantWith<object>(_)));
  auto const &obj = *std::get<object>(*root);
  EXPECT_THAT(obj, UnorderedElementsAre(Pair(u8"a"s, VariantWith<u8string>(u8"c"s))));
}
// NOLINTNEXTLINE
...
TEST_F(RelativePointerBazStart, Zero) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"0"), Optional(VariantWith<u8string>(u8"baz"s)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerBazStart, OneSlashZero) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"1/0"), Optional(VariantWith<u8string>(u8"bar"s)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerBazStart, ZeroMinusOne) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"0-1"), Optional(VariantWith<u8string>(u8"bar"s)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerBazStart, TwoHighlyNestedObjects) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"2/highly/nested/objects"), Optional(VariantWith<bool>(true)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerBazStart, ZeroHash) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"0#"), Optional(VariantWith<std::int64_t>(1)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerBazStart, ZeroMinusOneHash) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"0-1#"), Optional(VariantWith<std::int64_t>(0)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerBazStart, OneHash) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"1#"), Optional(VariantWith<u8string>(u8"foo"s)));
}
...
TEST_F(RelativePointerObjectsTrueStart, ZeroObjects) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"0/objects"), Optional(VariantWith<bool>(true)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerObjectsTrueStart, OneNestedObjects) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"1/nested/objects"), Optional(VariantWith<bool>(true)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerObjectsTrueStart, TwoFooZero) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"2/foo/0"), Optional(VariantWith<u8string>(u8"bar"s)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerObjectsTrueStart, ZeroHash) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"0#"), Optional(VariantWith<u8string>(u8"nested"s)));
}
// NOLINTNEXTLINE
TEST_F(RelativePointerObjectsTrueStart, OneHash) {
  EXPECT_THAT(start_->eval_relative_pointer(u8"1#"), Optional(VariantWith<u8string>(u8"highly"s)));
}
