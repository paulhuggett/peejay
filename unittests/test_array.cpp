...
TEST_F (JsonArray, TrailingCommaDisabled) {
  {
    ...
    EXPECT_EQ (p1.coordinate (), (coord{column{2U}, row{1U}}));
  }
  {
    parser<json_out_callbacks> p2;
    p2.input ("[1,]"s).eof ();
    EXPECT_EQ (p2.last_error (), make_error_code (error_code::expected_token));
    EXPECT_EQ (p2.coordinate (), (coord{column{4U}, row{1U}}));
  }
}
