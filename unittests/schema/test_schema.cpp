//===- unittests/schema/test_schema.cpp -----------------------------------===//
//*           _                           *
...
TEST(SchemaConst, NumberPassing) {
  EXPECT_EQ(check(parse(u8R"({ "const": 1234 })"sv), parse(u8"1234"sv)), std::error_code{});
}
...
TEST_F(SchemaProperties, HasPropertyPassing) {
  ...
  EXPECT_EQ(check(schema_, parse(u8R"({ "name": "Alice" })")), std::error_code{});
}
// NOLINTNEXTLINE
TEST_F(SchemaProperties, MissingPropertyPassing) {
  // valid - instance has name, which is a string.
  EXPECT_EQ(check(schema_, parse(u8R"({ "fullName": "Alice" })")), std::error_code{});
}
// NOLINTNEXTLINE
TEST_F(SchemaProperties, ArrayPassing) {
  // valid - instance is not an object, therefore `properties` isn't applicable.
  EXPECT_EQ(check(schema_, parse(u8R"([ "name", 123 ])")), std::error_code{});
}
// NOLINTNEXTLINE
...
TEST_F(SchemaMaxProperties, ObjectPassing) {
  EXPECT_EQ(check(schema_, parse(u8R"({ "a": 1, "b": 2 })")), std::error_code{});
}
// NOLINTNEXTLINE
