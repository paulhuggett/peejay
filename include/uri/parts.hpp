...
constexpr pctencode_set pctencode_set_from_parts_field(parts_field const field) noexcept {
  switch (field) {
...
template <typename T> class ro_sink_container {
public:
  using value_type = T;
...
template <std::output_iterator<char8_t> OutputIterator> struct puny_encoded_result {
  OutputIterator out;
...
template <std::ranges::input_range Range, std::output_iterator<char> OutputIterator>
  requires std::is_same_v<std::remove_cv_t<std::ranges::range_value_t<Range>>, char32_t>
puny_encoded_result<OutputIterator> puny_encoded(Range&& range, OutputIterator out) {
  ...
  for (;;) {
    std::string segment;
    auto const& [in, _, any_non_ascii] = punycode::encode(view, true, std::back_inserter(segment));
    if (any_non_ascii) {
...
template <std::ranges::input_range Range>
  requires std::is_same_v<std::remove_cv_t<typename std::ranges::range_value_t<Range>>, char32_t>
std::size_t puny_encoded_size(Range&& range) {
  details::ro_sink_container<char> sink;
  return puny_encoded(std::forward<Range>(range), std::back_inserter(sink)).any_non_ascii ? sink.size()
                                                                                          : std::size_t{0};
}

template <std::output_iterator<char8_t> OutputIterator> struct puny_decoded_result {
  OutputIterator out;
  bool any_encoded = false;
};

template <std::output_iterator<char8_t> OutputIterator>
puny_decoded_result(OutputIterator, bool) -> puny_decoded_result<OutputIterator>;

template <std::ranges::bidirectional_range Range, std::output_iterator<char8_t> OutputIterator>
  requires std::is_same_v<std::ranges::range_value_t<Range>, char>
std::variant<std::error_code, puny_decoded_result<OutputIterator>> puny_decoded(Range&& range, OutputIterator out) {
  using std::ranges::subrange;
  ...
  for (;;) {
    if (starts_with(view, idna_prefix)) {
      ...
      auto decode_result = punycode::decode(subrange{first, std::ranges::end(view)});
      using success_type = std::variant_alternative_t<1U, decltype(decode_result)>;
      if (auto const* const dr = std::get_if<success_type>(&decode_result)) {
        first = std::move(dr->in);
        out = std::ranges::copy(dr->str | icubaby::views::transcode<char32_t, char8_t>, out).out;
      } else if (auto const* const erc = std::get_if<std::error_code>(&decode_result)) {
        return *erc;
      } else {
...
std::variant<std::error_code, std::size_t> puny_decoded_size(Range&& range) {
  ...
  if (auto const* const decoded = std::get_if<puny_decoded_result<decltype(out)>>(&result)) {
...
template <typename VectorContainer>
  requires std::contiguous_iterator<typename VectorContainer::iterator> &&
           std::is_same_v<typename VectorContainer::value_type, char>
std::variant<std::error_code, parts> decode(VectorContainer& store, parts const& p) {
  using pft = std::underlying_type_t<parts_field>;
  std::bitset<static_cast<pft>(parts_field::last)> needs_encoding;
