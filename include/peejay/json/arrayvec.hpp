//===- include/peejay/json/arrayvec.hpp -------------------*- mode: C++ -*-===//
//*                                             *
...
template <typename T, std::size_t Size = 256 / sizeof(T)> class arrayvec : public details::avbase<T> {
public:
...
#ifndef NDEBUG
    T *const p = this->data();
    std::fill(pointer_cast<std::byte>(p + this->size()), pointer_cast<std::byte>(p + this->max_size()),
              std::byte{0xFF});
#endif
  }
