//===- include/peejay/json/emit.hpp -----------------------*- mode: C++ -*-===//
//*                 _ _    *
//*   ___ _ __ ___ (_) |_  *
//*  / _ \ '_ ` _ \| | __| *
//* |  __/ | | | | | | |_  *
...
// to char pointer
// ~~~~~~~~~~~~~~~
template <typename T> constexpr char const* to_char_pointer(T&& x) noexcept {
...
public:
  ...
  constexpr explicit indent(std::streamsize spaces) noexcept : spaces_{spaces} {}
  ...
  /// \returns \p os.
  template <typename OStream> OStream& write(OStream& os) const {
  ...
  [[nodiscard]] constexpr indent next() const noexcept { return indent{spaces_, depth_ + 1U}; }
...
private:
  ...
  constexpr indent(std::streamsize const spaces, unsigned const depth) noexcept : spaces_{spaces}, depth_{depth} {}
...
/// \param i  An indent.
/// \returns  \p os.
template <typename OStream> inline OStream& operator<<(OStream& os, indent const& i) {
  return i.write(os);
}
...
constexpr char to_hex(unsigned v) noexcept {
  ...
  return static_cast<char>(v + ((v < letter_threshold) ? '0' : 'A' - letter_threshold));
}

// break char
// ~~~~~~~~~~
inline u8string_view::const_iterator break_char(u8string_view::const_iterator first,
                                                u8string_view::const_iterator last) noexcept {
  return std::find_if(first, last, [](char8 const c) {
    return c < static_cast<char8>(char_set::space) || c == static_cast<char8>(char_set::quotation_mark) ||
...
template <typename OStream> void emit_object(OStream& os, indent i, object const& obj) {
  assert(obj);
...
template <typename OStream> void emit_array(OStream& os, indent i, array const& arr) {
  assert(arr);
...
template <typename OStream> void emit_string_view(OStream& os, u8string_view const& str) {
  os << '"';
  ...
  while ((pos = break_char(pos, last)) != last) {
    ...
    default: {
      ...
      std::array<char, 5> hex{{'u', '0', '0', to_hex(std::to_integer<unsigned>(high_nibble)),
...
// emit
// ~~~~
template <typename OStream> OStream& emit(OStream& os, indent const i, element const& el) {
  std::visit(overloaded{[&os](u8string const& s) { emit_string_view(os, s); }, [&os](int64_t v) { os << v; },
                        [&os](uint64_t v) { os << v; }, [&os](double v) { os << v; },
                        [&os](bool b) { os << (b ? "true" : "false"); }, [&os](null) { os << "null"; },
                        [&os, i](array const& arr) { emit_array(os, i, arr); },
                        [&os, i](object const& obj) { emit_object(os, i, obj); }, [](mark) { assert(false); }},
             el);
  return os;
}
...
///   level.
/// \returns  \p os.
template <typename OStream> OStream& emit(OStream& os, std::optional<element> const& root, std::streamsize spaces = 2) {
  if (root) {
