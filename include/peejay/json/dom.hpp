//===- include/peejay/json/dom.hpp ------------------------*- mode: C++ -*-===//
//*      _                  *
//*   __| | ___  _ __ ___   *
//*  / _` |/ _ \| '_ ` _ \  *
//* | (_| | (_) | | | | | | *
...
using variant = std::variant<std::int64_t, double, bool, null, u8string, array, object, mark>;
...
struct element : variant {
  ...
  element(element &&rhs) noexcept : variant(std::move(static_cast<variant &&>(rhs))) { adjust_parents(); }
  ~element() noexcept = default;
...
private:
  ...
  static std::pair<u8string, u8string_view::size_type> next_token(u8string_view s,
                                                                  u8string_view::size_type token_start);
...
inline element &element::operator=(element &&rhs) noexcept {
  if (this != &rhs) {
    static_cast<variant *>(this)->operator=(std::move(static_cast<variant &&>(rhs)));
    adjust_parents();
  }
  return *this;
}
...
inline std::optional<std::pair<u8string_view, unsigned>> decimal(u8string_view s) {
  auto prefix = 0U;
...
inline std::optional<variant> element::eval_relative_pointer(u8string_view s) {
  ...
  for (; prefix > 0U; --prefix) {
    ...
    if (std::holds_alternative<array>(*current->parent) || std::holds_alternative<object>(*current->parent)) {
  ...
  if (s[0] == '+' || s[0] == '-') {
    ...
    if (auto const *const arr = std::get_if<array>(current->parent)) {
      ...
      if (new_index < 0 || static_cast<udiff_type>(new_index) >= (*arr)->size()) {
        return {};
      }
  ...
  if (s == u8string_view{u8"#"}) {
    ...
    if (current->parent != nullptr) {
      ...
      if (auto const *const obj = std::get_if<object>(current->parent)) {
        ...
        if (auto const pos =
                std::find_if((*obj)->begin(), end, [current](auto const &kvp) { return &kvp.second == current; });
            pos != end) {
          return pos->first;
        }
      }
...
// next token
// ~~~~~~~~~~
inline std::pair<u8string, u8string_view::size_type> element::next_token(u8string_view s,
                                                                         u8string_view::size_type token_start) {
  auto const token_end = s.find('/', token_start);
  u8string_view const token_view =
      s.substr(token_start, token_end != u8string_view::npos ? token_end - token_start : u8string_view::npos);
...
///   peejay::element.
template <std::size_t StackSize> class dom {
public:
  ...
  std::error_code key(u8string_view const &s) { return this->string_value(s); }
...
template <std::size_t StackSize = 1024> dom() -> dom<StackSize>;

// string
// ~~~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::string_value(icubaby::u8string_view const &s) {
...
// integer
// ~~~~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::integer_value(std::int64_t v) {
  if (stack_->size() >= stack_size) {
    return error::dom_nesting_too_deep;
  }
  stack_->emplace(v);
...
// double
// ~~~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::double_value(double v) {
  if (stack_->size() >= stack_size) {
    return error::dom_nesting_too_deep;
  }
  stack_->emplace(v);
...
// boolean
// ~~~~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::boolean_value(bool v) {
  if (stack_->size() >= stack_size) {
    return error::dom_nesting_too_deep;
  }
  stack_->emplace(v);
...
// null
// ~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::null_value() {
  if (stack_->size() >= stack_size) {
    return error::dom_nesting_too_deep;
  }
  stack_->emplace(null{});
...
// begin array
// ~~~~~~~~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::begin_array() {
  if (stack_->size() >= stack_size) {
    return error::dom_nesting_too_deep;
  }
  stack_->emplace(mark{});
...
// end array
// ~~~~~~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::end_array() {
...
// end object
// ~~~~~~~~~~
template <std::size_t StackSize> std::error_code dom<StackSize>::end_object() {
  assert(this->elements_until_mark() % 2U == 0U);
  typename object::element_type::size_type const size = this->elements_until_mark() / 2U;
  ...
  for (;;) {
    ...
    obj->try_emplace(std::move(std::get<icubaby::u8string>(key)), std::move(value));
    stack_->pop();
  }
...
// elements until mark
// ~~~~~~~~~~~~~~~~~~~
template <std::size_t StackSize> std::size_t dom<StackSize>::elements_until_mark() const noexcept {
  auto rend = std::rend(*stack_);
  auto pos = std::find_if(std::rbegin(*stack_), rend, [](element const &v) { return std::holds_alternative<mark>(v); });
  if (pos == rend) {
    return 0;
  }
