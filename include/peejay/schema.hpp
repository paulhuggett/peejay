...
bool is_integer (element const &el) {
...
error_or<bool> check_type (u8string const &type_name, element const &instance) {
...
error_or<bool> check_type (element const &type_name, element const &instance) {
...
error_or<bool> check (element const &schema, element const &instance) {
  ...
  if (auto const &type_pos = map.find (u8"type"); type_pos != end) {
    ...
    if (auto const *const name_array = std::get_if<array> (&type_pos->second)) {
  ...
  if (auto const &properties = map.find (u8"properties"); properties != end) {
    if (auto *const obj = std::get_if<object> (&properties->second)) {
      auto const &pmap = **obj;
    } else {
