//===- include/peejay/uri/pctdecode.hpp -------------------*- mode: C++ -*-===//
//*             _      _                    _       *
...
template <std::integral ValueT> constexpr std::byte hex2dec(ValueT const digit) noexcept {
  if (digit >= 'a' && digit <= 'f') {
...
template <typename Iterator> Iterator increment(Iterator pos, Iterator end) {
  ...
  std::advance(pos, remaining >= 3 && *pos == '%' && !either_bad(hex2dec(*(pos + 1)), hex2dec(*(pos + 2))) ? 3 : 1);
  return pos;
}
...
template <std::input_iterator InputIterator> bool needs_pctdecode(InputIterator first, InputIterator last) {
  return std::find_if(first, last, [](auto c) { return c == '%'; }) != last;
}

template <std::ranges::input_range View>
  requires std::ranges::forward_range<View>
class pctdecode_view : public std::ranges::view_interface<pctdecode_view<View>> {
public:
  class iterator;
  class sentinel;
...
template <typename Range> pctdecode_view(Range&&) -> pctdecode_view<std::views::all_t<Range>>;
...
public:
  ...
  constexpr iterator(pctdecode_view const& parent, std::ranges::iterator_t<View> current)
      : parent_{std::addressof(parent)}, pos_{std::move(current)} {}

  constexpr std::ranges::iterator_t<View> const& base() const& noexcept { return pos_; }
  constexpr std::ranges::iterator_t<View> base() && { return std::move(pos_); }

  constexpr std::ranges::range_reference_t<View> operator*() const {
    return details::deref<std::ranges::range_reference_t<View>>(pos_, std::ranges::end(parent_->base_), &hex_);
  }
  ...
  friend constexpr std::ranges::range_rvalue_reference_t<View> iter_move(iterator const& it) noexcept(
      noexcept(std::ranges::iter_move(it.pos_))) {
    return std::ranges::iter_move(it.pos_);
  }

  friend constexpr void iter_swap(iterator const& x,
                                  iterator const& y) noexcept(noexcept(std::ranges::iter_swap(x.pos_, y.pos_)))
...
private:
  ...
  [[no_unique_address]] std::ranges::iterator_t<View> pos_ = std::ranges::iterator_t<View>();
...
public:
  sentinel() = default;
  constexpr explicit sentinel(pctdecode_view& parent) : end_{std::ranges::end(parent.base_)} {}
...
struct pctdecode_range_adaptor {
  template <std::ranges::viewable_range Range> constexpr auto operator()(Range&& r) const {
...
template <typename Iterator> class pctdecode_iterator {
public:
  using iterator_category = std::forward_iterator_tag;
  ...
  constexpr pctdecode_iterator(Iterator first, Iterator last) : pos_{first}, end_{last} {}

  constexpr bool operator==(pctdecode_iterator const& other) const noexcept {
    assert(end_ == other.end_ && "Comparing iterators that refer to different inputs");
    return pos_ == other.pos_ && end_ == other.end_;
  }

  reference operator*() const { return details::deref<reference>(pos_, end_, &hex_); }
...
template <typename Iterator> pctdecode_iterator(Iterator, Iterator) -> pctdecode_iterator<Iterator>;

template <typename Container> constexpr auto pctdecode_begin(Container const& c) noexcept {
  return pctdecode_iterator{std::begin(c), std::end(c)};
}
template <typename Container> constexpr auto pctdecode_end(Container const& c) noexcept {
  return pctdecode_iterator{std::end(c), std::end(c)};
}

namespace details {
template <typename T>
constexpr auto has_begin_end(int) -> decltype(begin(std::declval<T&>()), end(std::declval<T&>()), std::true_type{});

template <typename T> constexpr auto has_begin_end(...) -> std::false_type;

template <typename T> inline constexpr bool has_begin_end_v = decltype(has_begin_end<T>(0))::value;
}  // namespace details

template <typename Iterator> class pctdecoder {
public:
  ...
  constexpr pctdecoder(Iterator begin, Iterator end) noexcept
      : begin_{pctdecode_iterator(begin, end)}, end_{pctdecode_iterator(end, end)} {}
...
template <typename Container> pctdecoder(Container) -> pctdecoder<typename Container::const_iterator>;
